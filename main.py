...
AIRPORT_QUIZ_QUESTIONS = (
	(
		...
		(
			"可以利用资源营利，只要不向外转发或评论",
			"不能利用资源营利，但可以私下转发给熟人",
			"不能向外转发资源，但可以公开批评他人的喜好",
			"不利用资源营利、不向外转发，也不随意评判他人",
			"只有小众资源不能外传，其他资源可以自由分享",
		),
		3,
	),
	(
		...
		(
			"塔台是群内唯一的主人，成员只需服从塔台安排",
			"成员通过真诚发言或分享资源，共同参与群内交流",
			"塔台必须监督每一则消息，并立即处理所有争议",
			"成员只需领取资源，不需要参与发言或分享",
			"群内的分享与参与应以付费交易为主要方式",
		),
		1,
	...
	(
		...
		(
			"塔台必须判断资源的原创归属，并裁决成员间的所有纠纷",
			"机器人发生故障时，塔台必须立即修复并补偿所有损失",
			"成员被移出群组后，塔台必须公开提供完整说明",
			"群内事务完全不需要管理，违规行为也不必处理",
			"塔台应作为判官，对成员之间的是非作出最终裁决",
			"违反核心价值观的成员可能被移出，且不一定另作解释",
		),
		5,
	),
	(
	...
	(
		...
		(
			"不同系列的资源可以混在同一批上传，数量不受限制",
			"不同系列应分批上传，每一批最多上传 10 个媒体",
			"为了增加有效期，可以重复上传无效或相同的资源",
			"可以上传清水图或视频换取正太资源",
			"媒体上传后不需要检查内容是否有效或属于同一系列",
		),
		1,
	),
	(
		"关于塔台(机器人)、飞机场(频道)与航站大厅(群组)的作用，以下哪一项说明正确？",
		(
			"塔台用于分享资源以及进入群组；飞机场用于成员闲聊；航站大厅用于提交资源",
			"塔台负责审核所有群内发言；飞机场与航站大厅都只用于接收系统通知",
			"塔台用于查分，进群与分享资源；飞机场用于查看并获取航班资源；航站大厅用于交流发言并延长通行证",
			"塔台只用于查询通行证；飞机场用于上传资源；航站大厅用于下载资源",
			"塔台、飞机场与航站大厅功能完全相同，可以任意选择其中一个使用",
		),
		2,
	),
)
